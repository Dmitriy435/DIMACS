...
# Determining characters of K^x make up bad chars of B
print("")
for i in range(len(badCharsB)):
    print("This is bad character " + str(i) + ", and the following are the two values of chi_1 and chi_2 on the generator:")
    print(badCharsB[i](test1))
    print(badCharsB[i](test2))
    print("")
...
for i, j, k in cartesian_product((range(q+1), range(q+1), range(q))):
    v1 = Vinduced.basis()[i]
    v2 = Vinduced.basis()[j]
    v3 = Vqdim3.basis()[k]
